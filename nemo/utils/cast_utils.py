device = get_current_device()# Copyright (c) 2022, NVIDIA CORPORATION.  All rights reserved.
#
...
def avoid_bfloat16_autocast_context():
    ...
    if torch.is_autocast_enabled() and torch.get_autocast_gpu_dtype() == torch.bfloat16:
        return torch.amp.autocast(get_current_device_type(), dtype=torch.float32)
    else:
        return nullcontext()
...
def avoid_float16_autocast_context():
    ...
    if torch.is_autocast_enabled() and torch.get_autocast_gpu_dtype() == torch.float16:
        ...
        if torch.cuda.is_bf16_supported():
            return torch.amp.autocast(get_current_device_type(), dtype=torch.bfloat16)
        else:
            return torch.amp.autocast(get_current_device_type(), dtype=torch.float32)
    else:
        return nullcontext()
